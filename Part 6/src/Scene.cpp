...
#pragma region Camera
    ...
    auto parseOneCamera = [&](const json &j) -> Camera {
        ...
        if (j.contains("_type") && j["_type"] == "lookAt") {
            ...
            if (j.contains("GazePoint")) {
                c.gazePoint = parseVec3(j.at("GazePoint").get<std::string>());
                c.gaze = c.gazePoint.subtract(c.position).normalize();
            }
            else if (j.contains("Gaze")) {
                c.gaze = parseVec3(j.at("Gaze").get<std::string>()).normalize();
            }
        }
        ...
        if (j.contains("Renderer")) {
            std::string r = j.at("Renderer").get<std::string>();
            if (r == "PathTracing") c.renderer = Camera::RendererType::PathTracing;
            else c.renderer = Camera::RendererType::RayTracing;
        } else {
            c.renderer = Camera::RendererType::RayTracing;
        }

        auto parseRendererParams = [&](const std::string &s, Camera &c) {
            std::istringstream iss(s);
            std::string tok;
            while (iss >> tok) {
                if (tok == "ImportanceSampling")
                    c.pathTracingOptions.importanceSampling = true;

                else if (tok == "NextEventEstimation")
                    c.pathTracingOptions.nextEventEstimation = true;

                else if (tok == "RussianRoulette")
                    c.pathTracingOptions.russianRoulette = true;

                else if (tok == "MIS_BALANCE")
                    c.pathTracingOptions.mis = Camera::MISHeuristic::Balance;

                else if (tok == "MIS_POWER")
                    c.pathTracingOptions.mis = Camera::MISHeuristic::Power;

                else if (tok == "MIS_01")
                    c.pathTracingOptions.mis = Camera::MISHeuristic::OneSample;
            }
        };

        if (j.contains("RendererParams")) {
            std::string params = j.at("RendererParams").get<std::string>();
            if (!params.empty())
                parseRendererParams(params, c);
        }

        c.maxRecursionDepth = this->maxRecursionDepth;
        c.minRecursionDepth = 0;
        c.splittingFactor = 1;

        if (j.contains("MaxRecursionDepth"))
            c.maxRecursionDepth = std::stoi(j.at("MaxRecursionDepth").get<std::string>());

        if (j.contains("MinRecursionDepth"))
            c.minRecursionDepth = std::stoi(j.at("MinRecursionDepth").get<std::string>());
        else
            c.minRecursionDepth = 0;

        if (j.contains("SplittingFactor"))
            c.splittingFactor = std::stoi(j.at("SplittingFactor").get<std::string>());
        else
            c.splittingFactor = 1;
    ...
    if (doc["Scene"]["Lights"].contains("AreaLight")) {
        ...
        auto parseOneAreaLight = [&](const json &j) {
            ...
            if (j.contains("Intensity")) {
                light.intensity = parseVec3(j["Intensity"].get<std::string>());
                light.radiance = light.intensity;
            } else if (j.contains("Radiance")) {
                light.radiance = parseVec3(j["Radiance"].get<std::string>());
                light.intensity = light.radiance;
            } else {
                light.intensity = Vec3(0, 0, 0);
                light.radiance = Vec3(0, 0, 0);
            }
...
#pragma region BRDF
    ...
    if (doc["Scene"].contains("BRDFs")) {
        const auto &brdfRoot = doc["Scene"]["BRDFs"];

        if (brdfRoot.is_object()) {
            for (auto it = brdfRoot.begin(); it != brdfRoot.end(); ++it) {
                const std::string brdfTypeStr = it.key();
                const auto &node = it.value();

                auto parseOneBRDF = [&](const json &j) {
                    BRDF b;
                    b.type = brdfTypeFromString(brdfTypeStr);

                    if (j.contains("_id")) {
                        b.id = std::stoi(j["_id"].get<std::string>());
                    }

                    if (j.contains("Exponent")) {
                        b.exponent = std::stof(j["Exponent"].get<std::string>());
                    }

                    if (j.contains("_normalized")) {
                        b.normalized = parseBool(j["_normalized"]);
                    }

                    if (j.contains("kdfresnel")) {
                        b.kdFresnel = parseBool(j["kdfresnel"]);
                    }
                    else if (j.contains("_kdfresnel")) {
                        b.kdFresnel = parseBool(j["_kdfresnel"]);
                    }

                    this->brdfs.push_back(b);
                };

                if (node.is_array()) {
                    for (const auto &j: node) parseOneBRDF(j);
...
#pragma region Materials
    ...
    auto parseMaterial = [&](const json &materialJson) {
        ...
        material.brdfId = -1;
        if (materialJson.contains("_BRDF")) {
            material.brdfId = std::stoi(materialJson["_BRDF"].get<std::string>());
        }
...
#pragma region Vertex Data
    ...
    if (doc["Scene"].contains("VertexData")) {
        const auto &vd = doc["Scene"]["VertexData"];
        std::string vertexDataStr;
        std::string binFile;

        if (vd.is_object()) {
            if (vd.contains("_data"))
                vertexDataStr = vd["_data"].get<std::string>();
            else if (vd.contains("_binaryFile"))
                binFile = vd["_binaryFile"].get<std::string>();
        }
        else {
            vertexDataStr = vd.get<std::string>();
        }

        if (!binFile.empty()) {
            std::ifstream bin(binFile, std::ios::binary);
            if (!bin) {
                std::cerr << "Binary file not found. " << binFile << std::endl;
            }
            else {
                int vertexCount = 0;
                bin.read(reinterpret_cast<char *>(&vertexCount), sizeof(int));

                if (vertexCount <= 0) {
                    std::cerr << "VertexData invalid count. " << binFile << std::endl;
                } else {
                    std::vector<float> floats(3ull * vertexCount);
                    bin.read(reinterpret_cast<char *>(floats.data()), sizeof(float) * floats.size());

                    this->vertexData.reserve(vertexCount);
...
#pragma region TexCoordData
    if (doc["Scene"].contains("TexCoordData")) {
        const auto &tcd = doc["Scene"]["TexCoordData"];
        std::string texStr;
        std::string binFile;

        if (tcd.is_object()) {
            if (tcd.contains("_data"))
                texStr = tcd["_data"].get<std::string>();
            else if (tcd.contains("_binaryFile"))
                binFile = tcd["_binaryFile"].get<std::string>();
        }
        else {
            texStr = tcd.get<std::string>();
        }
        ...
        if (!binFile.empty()) {
            ...
            if (!bin) {
                ...
                int uvCount = 0;
                bin.read(reinterpret_cast<char *>(&uvCount), sizeof(int));

                if (uvCount <= 0) {
                    std::cerr << "TexCoordData invalid count. " << binFile << std::endl;
                }
                else {
                    std::vector<float> floats(2ull * uvCount); // 2 floats per UV
                    bin.read(reinterpret_cast<char *>(floats.data()), sizeof(float) * floats.size());

                    this->texCoordData.reserve(uvCount);
                    for (int i = 0; i < uvCount; ++i) {
                        this->texCoordData.emplace_back(
                            floats[2 * i + 0],
                            floats[2 * i + 1]
                        );
                    }
...
#pragma region Objects
    ...
    // Light Spheres
    if (objectData.contains("LightSphere")) {
        auto lsData = objectData["LightSphere"];

        auto processLightSphere = [&](const json &j) {
            LightSphere s;
            s.id = stoi(j["_id"].get<string>());
            s.materialId = stoi(j["Material"].get<string>());
            s.center = stoi(j["Center"].get<string>());
            s.radius = stof(j["Radius"].get<string>());
            s.radiance = parseVec3(j["Radiance"].get<string>());

            parseTransformations(j, s.transformationOrder);
            parseTextureIds(j, s.textureMapIds);

            if (j.contains("MotionBlur"))
                s.motionBlur = parseVec3(j["MotionBlur"].get<string>());
            else
                s.motionBlur = Vec3(0.0f, 0.0f, 0.0f);

            this->objects.lightSpheres.push_back(s);
        };

        if (lsData.is_array()) {
            for (const auto &j: lsData)
                processLightSphere(j);
        }
        else if (lsData.is_object()) {
            processLightSphere(lsData);
        }
    }
    ...
    if (objectData.contains("Mesh")) {
        ...
        bool globalZeroBased = false;
        if (doc["Scene"].contains("ZeroBasedIndexing")) {
            globalZeroBased = parseBool(doc["Scene"]["ZeroBasedIndexing"]);
        }
        ...
        auto processMesh = [&](const json &meshJson) {
            ...
            const auto& facesJson = meshJson["Faces"];

            int fileVertexOffset = 0;
            if (facesJson.contains("_vertexOffset")) {
                fileVertexOffset = std::stoi(facesJson["_vertexOffset"].get<std::string>());
                mesh.vertexOffset = fileVertexOffset;
            }

            if (facesJson.contains("_textureOffset")) {
                mesh.textureOffset = std::stoi(facesJson["_textureOffset"].get<std::string>());
            }

            int loadingOffset = fileVertexOffset;
            if (globalZeroBased) {
                loadingOffset += 1;
            }

            if (facesJson.contains("_data")) {
                std::stringstream issFaces(facesJson["_data"].get<std::string>());
                int idx;
                while (issFaces >> idx) {
                    mesh.data.push_back(idx + loadingOffset);
                }
            }
            // Binary File (.bin)
            else if (facesJson.contains("_binaryFile")) {
                std::string binPath = facesJson["_binaryFile"].get<std::string>();
                std::ifstream bin(binPath, std::ios::binary);

                if (!bin) {
                    std::cerr << "Faces binary file not found. " << binPath << std::endl;
                } else {
                    int faceCount = 0;
                    bin.read(reinterpret_cast<char *>(&faceCount), sizeof(int));

                    if (faceCount <= 0) {
                        std::cerr << "Faces invalid count. " << binPath << std::endl;
                    } else {
                        std::vector<int> indices(3ull * faceCount);
                        bin.read(reinterpret_cast<char *>(indices.data()), sizeof(int) * indices.size());

                        mesh.data.reserve(mesh.data.size() + indices.size());
                        for (int idx: indices) {
                            mesh.data.push_back(idx + loadingOffset);
                        }
                    }
            ...
            // PLY File (.ply)
            else if (facesJson.contains("_plyFile")) {
                string plyPath = facesJson["_plyFile"].get<std::string>();
            ...
            if (meshJson.contains("Transformations")) {
                parseTransformations(meshJson, mesh.transformationOrder);
            }
            ...
            if (meshJson.contains("MotionBlur"))
                ...
                mesh.motionBlur = Vec3(0.0f, 0.0f, 0.0f);
    ...
    // Light Meshes
    if (objectData.contains("LightMesh")) {
        auto lmData = objectData["LightMesh"];

        auto processLightMesh = [&](const json &j) {
            LightMesh mesh;
            mesh.id = stoi(j["_id"].get<string>());
            mesh.materialId = stoi(j["Material"].get<string>());
            mesh.radiance = parseVec3(j["Radiance"].get<string>()); // Emission color

            if (j.contains("_shadingMode")) {
                if (j["_shadingMode"].get<string>() == "smooth")
                    mesh.shadingMode = ShadingMode::Smooth;
                else
                    mesh.shadingMode = ShadingMode::Flat;
            } else {
            ...
            if (j.contains("Faces") && j["Faces"].is_object()) {
                const auto &F = j["Faces"];

                if (F.contains("_vertexOffset"))
                    mesh.vertexOffset = stoi(F["_vertexOffset"].get<string>());

                if (F.contains("_textureOffset"))
                    mesh.textureOffset = stoi(F["_textureOffset"].get<string>());

                if (F.contains("_data")) {
                    std::stringstream issFaces(F["_data"].get<string>());
                    int idx;

                    int vOff = 0;
                    if (F.contains("_vertexOffset"))
                        vOff = stoi(F["_vertexOffset"].get<string>());

                    while (issFaces >> idx) {
                        if (vOff > 0) mesh.data.push_back(idx + vOff);
                        else mesh.data.push_back(idx);
                    }
                } else if (F.contains("_plyFile")) {
                    std::string plyPath = F["_plyFile"].get<std::string>();
                    ...
                    if (readPLY(plyPath, vertices, indices, plyUVs)) {
                        ...
                        for (int ii: indices)
                            mesh.data.push_back(static_cast<int>(baseIndex + ii));
                    } else {
                        cerr << "Failed to read PLY file: " << plyPath << std::endl;
                    }
                }
            ...
            parseTransformations(j, mesh.transformationOrder);
            parseTextureIds(j, mesh.textureMapIds);

            if (j.contains("MotionBlur"))
                mesh.motionBlur = parseVec3(j["MotionBlur"].get<string>());
            else
                mesh.motionBlur = Vec3(0.0f, 0.0f, 0.0f);

            this->objects.lightMeshes.push_back(mesh);
        };

        if (lmData.is_array()) {
            for (const auto &j: lmData) processLightMesh(j);
        } else if (lmData.is_object()) {
            processLightMesh(lmData);
        }
    }

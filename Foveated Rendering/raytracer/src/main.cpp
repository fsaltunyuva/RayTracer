...
#pragma region Foveated Rendering
...
#pragma region Eccentricity Calculation - Parameters
                    ...
                    float eccentricity = angleRad * 180.0f / (float) M_PI; // Convert to degrees


                    float foveaRadius = 20.0f;
                    float blendRadius = 30.0f;
                    int maxSamples = cam.numSamples;
                    int blendSamples = 16;
                    int minSamples = 1;
                    int N_foveated = minSamples;
#pragma endregion

#pragma region Static Foveated Rendering
                    if (useStaticFoveatedRendering) {
                        // int flippedYDebug = height - 1 - y;
                        if (eccentricity <= foveaRadius) {
                            N_foveated = maxSamples;
                            // DEBUG: RED
                            // hdr[flippedYDebug * width + x] = Vec3(255.0f, 0.0f, 0.0f);
                            // continue;
                        }
                        else if (eccentricity <= blendRadius) {
                            N_foveated = blendSamples;
                            // DEBUG: GREEN
                            // hdr[flippedYDebug * width + x] = Vec3(0.0f, 255.0f, 0.0f);
                            // continue;
                        }
                        else {
                            N_foveated = minSamples;
                            // DEBUG: BLUE
                            // hdr[flippedYDebug * width + x] = Vec3(0.0f, 0.0f, 255.0f);
                            // continue;
                        }
                    }
#pragma endregion

#pragma region Log Acuity Falloff with Fovea-Blend-Peripheral Regions
                    if (useLogAcuityFalloffWithRegions) {
                        if (eccentricity <= foveaRadius) {
                            N_foveated = maxSamples;
                        }
                        else if (eccentricity <= blendRadius) {
                            // N = N_max * (e0 / (e + e0))^2
                            // Shift eccentricity so that at foveaRadius shiftedEccentricity = 0
                            float shiftedEccentricity = eccentricity - foveaRadius;
                            float local_e0 = 10.0f;
                            float term = local_e0 / (shiftedEccentricity + local_e0);
...
#pragma region Log Acuity Falloff From Center to Blend Edge
                    if (useLogAcuityFalloffFromCenter) {
                        float e0 = 5.0f;

                        if (eccentricity > blendRadius) {
                            N_foveated = minSamples;
                        }
                        else {
                            // N = N_max * (e0 / (e + e0))^2
...
#pragma region Linear Acuity Falloff From Center to Blend Edge
                    if (useLinearAcuityFalloffFromCenter) {
                        float a = 0.02f; // MAR(0)
...
#pragma region Mixed Acuity Falloff From Center to Blend Edge
                    if (useMixedAcuityFalloffFromCenter) {
                        if (eccentricity > blendRadius) {
                            N_foveated = minSamples;
                        }
                        else {
...
#pragma endregion
                    ...
                    // Set N for the loop below
                    if (useStaticFoveatedRendering || useLogAcuityFalloffWithRegions ||
                        useLogAcuityFalloffFromCenter || useLinearAcuityFalloffFromCenter ||
                        useMixedAcuityFalloffFromCenter)
                        N = N_foveated;
                    else
                        N = cam.numSamples; // Default
...
#pragma endregion
                    ...
                    if (N == 1) {
                        ...
                        std::shuffle(xCoords.begin(), xCoords.end(), rng);
                        std::shuffle(yCoords.begin(), yCoords.end(), rng);

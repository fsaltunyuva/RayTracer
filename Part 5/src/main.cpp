...
static Vec3 sampleHemisphere(const Vec3& N, EnvSampler sampler, float u1, float u2, float& pdfOut) {
    ...
    if (sampler == EnvSampler::Uniform) {
        ...
        float zLocal = u1;
        float r = std::sqrt(std::max(0.0f, 1.0f - zLocal*zLocal));
        ...
        x = r * std::cos(phi);
        y = r * std::sin(phi);
        z = std::sqrt(std::max(0.0f, 1.0f - u1));
...
Vec3 traceRay(const Scene& scene, const Vec3& rayOrigin, const Vec3& rayDir, int depth, Intersector& intersector, float rayTime, const Vec2& bgUV) {
    ...
    for (const auto& sl : scene.spotLights) {
        ...
        float NdotL = std::max(0.0f, hitNormal.dot(L));
        Vec3 diffuse = kd.multiply(effIntensity).scale(NdotL);

        Vec3 viewDir = rayOrigin.subtract(hitPoint).normalize();
        Vec3 halfVec = L.add(viewDir).normalize();
        float NdotH = std::max(0.0f, hitNormal.dot(halfVec));
        Vec3 specular = ks.multiply(effIntensity).scale(std::pow(NdotH, material.phongExponent));

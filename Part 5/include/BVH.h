...
public:
    ...
    bool intersect(const Vec3& rayOrigin, const Vec3& rayDir, float& closestT, IntersectionInfo& info, float epsilon) const; // Ray-BVH intersection

private:
    BVHNode* buildRecursive(const std::vector<int>& tris, int depth); // Recursive BVH builder
    bool intersectNode(BVHNode* node, const Vec3& rayOrigin, const Vec3& rayDir, float& closestT, IntersectionInfo& info, float epsilon) const; // Recursive intersection test
};
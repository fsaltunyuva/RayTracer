...
bool BVH::intersect(const Vec3& rayOrigin, const Vec3& rayDir, float& closestT, IntersectionInfo& info, float epsilon) const {
    Vec3 d = rayDir.normalize();
    return intersectNode(root, rayOrigin, d, closestT, info, epsilon);
}

bool BVH::intersectNode(BVHNode* node, const Vec3& rayOrigin, const Vec3& rayDir,
                        float& closestT, IntersectionInfo& info, float epsilon) const {
    ...
    bool hitLeft = intersectNode(node->left, rayOrigin, rayDir, closestT, info, epsilon);
    bool hitRight = intersectNode(node->right, rayOrigin, rayDir, closestT, info, epsilon);
